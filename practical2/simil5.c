...
{ int i,j,d,im,dm;// dm es la distancia del código más parecido, im su índice
  ...
  #pragma omp parallel
  {
    for ( j = 0 ; j < nt2 ; j++ ) {
      ...
      #pragma omp for private(i, d) schedule(static)
      //#pragma omp for private(i, d) schedule(static,1)
      //#pragma omp for private(i, d) schedule(dynamic,1)
...
{ int err,err2,nt1,nt2,*ps,numeros;
  ...
  // exercise 2 - read both files parallel and time for it
  t1 = omp_get_wtime();
