...
int main(int argc,char *argv[])
{
  ...
  /* COMPLETE: loop of NREPS iterations.
     ...
     for(i = 0; i<NREPS;i++){
       if(myid==0){
         MPI_Send(&buf[0],n,MPI_CHAR,1,0,MPI_COMM_WORLD);
         MPI_Recv(&buf[0],n,MPI_CHAR,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
       }else{
        MPI_Recv(&buf[0],n,MPI_CHAR,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        MPI_Send(&buf[0],n,MPI_CHAR,0,0,MPI_COMM_WORLD);
